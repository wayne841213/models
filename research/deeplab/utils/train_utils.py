...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    alpha = 0.25
    gamma = 2
    softmax = tf.reshape(tf.nn.softmax(logits), shape=[-1])
    prob = tf.gather(softmax, scaled_labels)
    loss_weight = tf.pow(tf.subtract(1., prob), gamma) 

    not_ignore_mask = tf.to_float(tf.not_equal(scaled_labels,
                                               ignore_label)) * loss_weight * alpha
